...
import { Category, IMakeup } from "../../../../src/interfaces";
			...
			<LayoutAdmin>
				...
				<FormCategory category={category} />
				{
					...
					<Link href={`/admin/sites/${router.query.category}/f/new`}>
						<a className="transition duration-150 ease-in-out hover:bg-pink-500 focus:outline-none border bg-pink-600 rounded text-white px-8 py-2 text-sm">Nuevo Descatado</a>
					</Link>
				</div>
				...
				<div className="my-6 container px-2 mx-auto flex flex-row lg:flex-row items-center lg:items-center justify-between ">
					...
					<Link href={`/admin/sites/${router.query.category}/new`}>
						<a className="transition duration-150 ease-in-out hover:bg-pink-500 focus:outline-none border bg-pink-600 rounded text-white px-8 py-2 text-sm">Nueva Sección</a>
					</Link>
				</div>
